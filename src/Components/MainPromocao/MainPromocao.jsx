  ...
  return(
    <main>
        <section className="container-fluid mb-2 text-center">
          <h1 className="font-config pt-3 texto">Promoções</h1>
          <FiltroPromocao/>
        </section>
        <div className="tela-promocao">
          {promocao.map((promocao) => {
            if(destinoEscolhido === promocao.destino.nomeDestino || destinoEscolhido === "Todas as Promoções"){ 
              return(
              <CardPromocao destino = {promocao.destino.nomeDestino} fotoPromocao={(promocao.destino.foto === ""? egito:promocao.destino.foto )} dataEmbarque={promocao.data} localEmbarque={promocao.localPartida} preco={promocao.precoPromocao}  key= {promocao.idPromocao}/>
            )}
          })} 
        </div>
      </main>
